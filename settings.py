...
# --- Видео ---
VIDEO_ENABLED = False                      # записывать видео при обнаружении человека
VIDEO_MAX_DURATION = 5                    # максимальная длина видео (секунд)
VIDEO_ON_UNKNOWN_ONLY = False              # записывать только неопознанных (иначе всех подряд)
VIDEO_FORMAT = "mkv"                      # формат: avi, mp4, mkv, mov, flv
VIDEO_DIR = Path("videos")                # папка для видео
SEND_VIDEO_EMAIL = False                   # отправлять видео по email
SEND_VIDEO_TELEGRAM = False                # отправлять видео в Telegram
